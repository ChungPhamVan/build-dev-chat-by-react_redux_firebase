...
class Channels extends Component {
  ...
  addChannel = () => {
    ...
    const key = channelsRef.push().key; // sinh ngau nhien 1 id
    const newChannel = {
      id: key,
      name: channelName,
  ...
  getNotificationCount = channel => {
    ...
    this.state.notifications.forEach(notification => {
      if (notification.id === channel.id) {
        count = notification.lastKnownTotal - notification.total;
      }
    });
    ...
    return (
      <React.Fragment>
        ...
        <Modal basic open={modal} onClose={this.closeModal} >
          ...
          <Modal.Content>
            <Form onSubmit={event => {this.handleSubmitted(event)}}>
              <Form.Field>
                <Input 
                  fluid
          ...
          <Modal.Actions>
            <Button color="green" inverted onClick={event => {this.handleSubmitted(event)}} >
